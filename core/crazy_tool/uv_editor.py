...
class MultiObjectUVEdit(bpy.types.Operator):
            ...
            if num_verts > 0:
                ...
                if len(tmp_obj.data.uv_textures) > 0:
                    if tmp_obj.data.uv_textures.active.name not in bpy.data.objects[v_group.name].data.uv_textures:
                        new_uv_layer = bpy.data.objects[v_group.name].data.uv_textures.new(
                            tmp_obj.data.uv_textures.active.name)
                        original_object.data.uv_textures.active = new_uv_layer
                    else:
                        original_object.data.uv_textures.active = original_object.data.uv_textures[
                            self.multi_object.data.uv_textures.active.name]
    ...
    def modal(self, context, event):
        if (event.type in [
            'TAB'] and not event.ctrl and not event.shift and not event.oskey) or self.multi_object.mode == "OBJECT":
